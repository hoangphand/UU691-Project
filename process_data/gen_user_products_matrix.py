...
# print(user_product_matrix_rdd.take(1))
...
# 		if keys[index] != 'User_ID':
# 			cates = list(keys[index])
# 			for index in range(0, len(cates)):
# 				file.write(str(cates[index]))
# 				if index != len(cates) - 1:
# 					file.write(',')
# 			file.write(':' + str(user[keys[index]]))
# 			if index != len(keys) - 1:
# 				file.write(';')
# 	file.write('\n')
# file.close()
